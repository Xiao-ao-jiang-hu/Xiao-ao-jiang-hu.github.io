...
## 4. RL-Based 分支改进说明 (强化学习驱动的团队组建)

在 v2 的 `RL-Based` 分支中，项目引入了基于强化学习的算法来优化科学家团队的组建过程。

### A. 强化学习驱动的合作伙伴选择
*   **可训练邻接矩阵 (`trainable_adjmatrix`)**: 引入了基于 PyTorch 的可训练参数矩阵，替代了原有的静态邻接矩阵。矩阵中的每个值表示两个科学家之间合作的潜力分数。
*   **策略梯度优化**: 系统将团队组建视为一个强化学习任务。
    *   **奖励机制**: 奖励基于团队在测试任务（如在指定问题上的表现）上的输出质量计算。
    *   **损失函数**: 使用策略梯度方法，损失函数为选定合作关系的对数概率与奖励的乘积，通过反向传播更新矩阵参数。

*   **Epsilon-Greedy 策略**: 在 `select_authors` 函数中实现了 $\epsilon$-greedy 采样。
