...
#### 结果
印证了之前提出的结论：如果不考虑网络延迟，则Q-CAST 的成功率明显优于普通 Dijkstra；如果考虑网络延迟，则两者Q-CAST 的成功率明显优于普通 Dijkstra，但是建立的端到端纠缠的保真度明显低于普通 Dijkstra。这是因为Q-CAST 的路由算法会尝试建立恢复路径，但这些路径的链路质量可能较差，并且链路会更长，经典信令的传输延迟更高。导致最终的保真度较低。这可能导致更多的纯化轮次等，从而降低了最终的吞吐量，并且对于保真度敏感的任务会有更大影响。