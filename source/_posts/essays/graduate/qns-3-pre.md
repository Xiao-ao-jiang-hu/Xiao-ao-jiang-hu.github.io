...
#### 内容总结
主要针对空地一体化量子网络。特点是自由空间信道（星地）受大气湍流影响，光纤信道（地面）受长度损耗影响，两者的噪声模型完全不同。此外卫星上的量子存储器极其珍贵，且窗口期非常短。核心思路是将动态的网络建模为一个时变图，并将其离散化为多个时间槽。目标是最大化全网端到端的纠缠分发吞吐量，约束是保真度大于阈值，中继器容量有限和纠缠交换必须在相干时间内完成。
...
实现网络层的快速重路由机制，当某一中继节点因环境噪声过载而失效时，网络层如何快速切换到备份路径并保持纠缠的一致性（论文中通过中心化+流量约束保证，但是在一般情形下可能不太适用，因为假设了地面边较稳定但空间边不稳定，并且空间边的质量是由经典物理数值计算出来的，而不是通过真正的测试）。在模拟干扰情形下比较静态路由与动态重路由在突发干扰下的连接中断率。

主要参考论文：[Dynamic Routing in Space-Ground Integrated Quantum Networks](https://arxiv.org/abs/2501.10252)

量子重路由会导致更多的一致性问题。

例如假设原路径请求需要 1 对纠缠，重路由后的新路径因为噪声大，需要 2 对纠缠进行纯化。中继节点在重路由切换瞬间，存储器里还残留着一对为旧路径准备的、已经存放了很久的纠缠比特。如果重路由逻辑没有严格的“强制清空”标识，节点可能会直接拿这对比特和新生成的比特进行纯化。由于旧比特已经发生了严重的退相干，纯化后的保真度反而比单对比特还要低。这导致最终分发给用户的 GHZ 态质量完全不达标，但控制器却认为它已经满足了 $N_{th}$ 约束。
