...
## **七、难点与易错点说明**
1. **向量平衡点 (Vector Balance Point)**  
   - **定义**：使向量/标量硬件利用率相等的向量代码比例。  
   - **计算**：若向量速度 $R_v = 90 \text{ MFLOPS}$，标量速度 $R_s = 10 \text{ MFLOPS}$，则平衡点为 90%（当程序90%为向量代码时硬件利用率最高）。

2. **链接技术时序**  
   - **关键约束**：仅当首元素**进入结果寄存器**时才能启动下条指令（同步点）。
   - **易错**：忽略数据通路延迟（如 `V2`结果写回后才能作为 `V3`的输入）。

3. **分段开采开销**  
   - **隐藏细节**：余数处理需单独循环，`T_loop`包含循环变量更新、分支跳转等开销。

4. **性能公式误区**  
   - $T_{\text{all}} = \left\lceil \frac{n}{\text{MVL}} \right\rceil (T_{\text{loop}} + T_{\text{start}}) + m \cdot n$ 中 `m·n` 项易遗漏（`m`是编队数，非分组数）。


## **八、总结**
本课程系统讲解了向量处理机的设计原理与优化技术：
1. **基础**：向量数据结构、处理方式（横向/纵向/纵横）。
2. **架构**：存储器-存储器 vs 寄存器-寄存器结构（Cray-1为典范）。
3. **优化**：链接技术、分段开采、条件执行、多处理机并行。
4. **评价**：处理时间、$R_\infty$、$n_{1/2}$、$n_v$ 的计算方法。
5. **实例**：从Cray-1到富岳，体现向量处理在高性能计算中的持续演进。

> **核心思想**：通过**专用硬件**（向量寄存器/流水线）和**软件技术**（编译优化）挖掘数据级并行，突破标量处理瓶颈。