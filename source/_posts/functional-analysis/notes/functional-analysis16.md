...
---
若 $X$ 是一个自反的Banach空间，则其对偶空间 $X^* = \mathcal{L}(X, \mathbb{R})$ 上的弱拓扑与弱\*拓扑一致，因此 $X^*$ 中的闭单位球是弱紧的，从而 $X$ 中的闭单位球也是弱紧的。Eberlein–Šmulian 定理断言，这一性质刻画了自反性。它还断言，闭单位球的弱紧性等价于序列弱紧性。

## Eberlein–Šmulian 定理

设 $X$ 为实Banach空间，令 $B := \{ x \in X \mid \|x\| \leq 1 \}$ 为闭单位球。则以下条件等价：
...
设 $C \subset X$ 为实Banach空间 $X$ 中的一个非空有界弱闭子集。则 $C$ 是弱紧的当且仅当 $X$ 上的每个有界线性泛函在 $C$ 上都能达到其最大值。
...
将 James 定理与 Eberlein–Šmulian 定理结合，可得到以下结果。Banach空间 $X$ 是自反的当且仅当，对于每个有界线性泛函 $x^* \in X^*$，存在元素 $x \in X$ 使得
$$
...
### 证明 (1) 蕴含 (2)
假设 $X$ 是自反的。则 $\iota: X \to X^{**}$ 是一个Banach空间等距映射，因此关于弱拓扑是一个同胚。由于 $X^*$ 是自反的，$X^{**}$ 上的弱拓扑与弱\*拓扑一致。因此，闭单位球 $B^{**} \subset X^{**}$ 是弱紧的，从而闭单位球 $B \subset X$ 也是弱紧的。这表明 (1) 蕴含 (2)。
