...
## 定理（开映射定理）
设 $X$ 和 $Y$ 为Banach空间，$T: X \to Y$ 为有界线性算子（即连续的线性算子）。若 $T$ 是满射的（即 $T(X) = Y$），则 $T$ 是开映射（即对任意 $X$ 中的开集 $U$，$T(U)$ 是 $Y$ 中的开集）。
...
2. 作为推论，可以得到逆算子定理：若 $T: X \to Y$ 是Banach空间之间的有界线性双射，则其逆算子 $T^{-1}: Y \to X$ 也是有界线性的。
3. 完备性不可移除
